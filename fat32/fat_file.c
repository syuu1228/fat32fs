...
ssize_t fat_file_read(fat_file * file, void *buffer, size_t c)
{
	...
	if (!c)
		return count;
	if (cluster_data_read (file->ins, file->chain->cluster_no, buffer, offset,
			c) != c)
		return -1;
...
{
	fat_cluster_chain *chain = LIST_GET(list_head (&(file->chain->list)), list,
			fat_cluster_chain);
	assert(chain);
...
bool fat_file_next_cluster(fat_file * file)
{
